package io.arkeus.tiled {
   ...
   * parsed layers.
   */
  public class TiledMap extends BaseLoader {
    ...
    /** A container containing information on the layers of the map. */
      ...
      public function TiledMap(loadPath:String, tmx:XML) {
			...
			tilesets = new TiledTilesets(loadPath, tmx.tileset);
			parseLayers(tmx);
		}

    public override function load(timeout:uint=0):void {
			tilesets.addEventListener(Event.COMPLETE, tilesetLoaded);
      tilesets.load();
    }

    protected function tilesetLoaded(evt:Event):void {
      loadComplete(this);
    }
